...
<div class="py-6">
    <div class="flex justify-end">
        ...
        wire:navigate style="padding: 10px; background-color: #3b82f6; color: white; font-size: 0.875rem; font-weight: 600; border-radius: 0.375rem;">
         + Create Event
     </a>
    </div>
    ...
    @else
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            @foreach($events as $event)
                <div class="bg-white rounded-xl shadow-md hover:shadow-lg p-6 transition-all duration-300 flex flex-col justify-between">
                    <div>
                        <div class="flex items-center justify-between mb-2">
                            <h3 class="text-xl font-semibold text-gray-900">{{ $event->title }}</h3>
                            <span class="inline-block bg-indigo-100 text-indigo-800 text-xs font-medium rounded-full px-2 py-1">
                                {{ $event->event_type }}
                            </span>
                        </div>
                        ...
                        <div class="text-gray-600 text-sm space-y-1 mb-4">
                            ...
                            <p><span class="font-medium">Time:</span> {{ \Carbon\Carbon::parse($event->time)->format('h:i A') }}</p>
                            <p><span class="font-medium">For:</span> {{ $event->eventFor->name }}</p>
                        </div>
                    </div>
